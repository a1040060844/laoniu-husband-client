...
- Needs device review: wife hero framing, status-card overlap on short phones, task publisher hero crop, review-card density, decree-card density, timeline density, gold tab touch feel, dark input contrast, picker values, form ergonomics, and review actions after repeated state changes.
...
- Unused full-page task/wife/reference images and unused loading/login static images were not kept in the baseline package; generated `dist` currently has 124 files, about 19.56 MB total, with `dist/assets` at about 19.04 MB.
