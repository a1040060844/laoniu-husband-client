...
export function WifeDashboard({
  ...
  const expPercent = Math.min(100, Math.round((progress.exp / requiredExp) * 100));
  ...
  const recentTask = submittedTasks[0] ?? tasks.find((task) => task.status === "doing" || task.status === "todo") ?? tasks[0];
  const pendingRulingCount = Math.max(2, submittedTasks.length);
  const pendingBenefitCount = 1;
  const abnormalCount = Math.max(1, tasks.filter((task) => task.status === "failed").length);
  ...
  function setWifePage(page: WifePage) {
    setSubPage(null);
    setActivePage(page);
    const nextHash = page === "main" ? "#wife-main" : page === "growth" ? "#wife-growth" : "#wife-today";
  ...
  function handleTouchEnd(event: TouchEvent<HTMLElement>) {
    ...
    const activeScroller = (event.target as HTMLElement | null)?.closest(".wife-growth, .wife-today");
    if (activeScroller instanceof HTMLElement && activePage !== "main") {
      const canScrollDown = activeScroller.scrollTop + activeScroller.clientHeight < activeScroller.scrollHeight - 2;
  ...
  function submitTask() {
    ...
    onCreateTask({
      id: taskId(),
      title,
      description: draft.description.trim() || "由老妞大人亲自发布，验收标准以老妞大人裁定为准。",
      type: draft.type,
      source: "wife",
  ...
  return (
    <section
      ...
      onWheel={handleWheel}
    >
      <div className="wife-pager-track" style={{ transform: `translateY(-${WIFE_PAGE_INDEX[activePage] * 100}dvh)` }}>
      <section className="wife-growth" id="wife-growth" aria-label="成长裁定">
        <img className="wife-growth__portrait" src="/assets/wife/wife-main.png" alt="" />
        ...
        <section className="wife-growth-card wife-growth-status" aria-label="老哥成长状态">
          <div className="wife-growth-status__head">
            ...
            <div className="wife-rank-seal" aria-label={`当前职务插画：${role.title}`}>
          ...
          <div className="wife-growth-progress" aria-label={`经验进度 ${expPercent}%`}>
        ...
        <section className="wife-growth-card wife-exp-control" aria-label="经验调整">
          <div className="wife-growth-section-title">
            <h2>经验调整</h2>
            <p>老妞大人可直接赏赐或扣除经验</p>
          </div>
          <div className="wife-exp-board">
            <button className="wife-exp-button wife-exp-button--danger" type="button" onClick={() => onAdjustExperience(-10)}>
              -10
            </button>
            <button className="wife-exp-button wife-exp-button--danger" type="button" onClick={() => onAdjustExperience(-5)}>
              -5
            </button>
            ...
            <button type="button" className="wife-exp-button" onClick={() => onAdjustExperience(5)}>
              +5
            </button>
            <button type="button" className="wife-exp-button wife-exp-button--strong" onClick={() => onAdjustExperience(10)}>
              +10
            </button>
          </div>
          <button className="wife-custom-adjust" type="button" onClick={customAdjust}>
            <Edit3 size={18} />
            自定义调整
          </button>
        </section>

        <section className="wife-growth-card wife-level-control" aria-label="等级调整">
          <div className="wife-growth-section-title">
            <h2>等级调整</h2>
            <p>升职、降级、流放，皆由老妞决定</p>
          </div>
          <div className="wife-level-strip">
            <span>当前： Lv.{String(role.level).padStart(2, "0")} {role.title}</span>
            <i aria-hidden="true">›</i>
            <span>下一： Lv.{String(nextRole.level).padStart(2, "0")} {nextRole.title}</span>
          </div>
        ...
        <section className="wife-growth-card wife-punish-card" aria-label="惩罚状态">
          <div className="wife-growth-section-title">
            <h2>惩罚状态</h2>
            <p>严重失败时，由老妞大人执行最终裁定</p>
          </div>
          <button className="wife-punish-state" type="button" onClick={onPunishStatus}>
            <Shield size={32} />
            <span>
        ...
        <a className="wife-growth-cue" href="#wife-main" onClick={handlePageLink("main")}>
      ...
      <section className="wife-hero" id="wife-main" aria-label="老婆端主控台">
        <img className="wife-portrait" src="/assets/wife/wife-main.png" alt="" />
        ...
        <header className="wife-title">
          ...
          <a className="wife-side-hint" href="#wife-growth" onClick={handlePageLink("growth")}>
            <ShieldCheck size={16} />
            <span>下滑裁定老哥成长</span>
          </a>
        </header>
        ...
        <section className="wife-status-card" aria-label="老哥当前状态">
          ...
          <div className="wife-progress" aria-label={`经验进度 ${expPercent}%`}>
            <i style={{ width: `${expPercent}%` }} />
          </div>
        ...
        <nav className="wife-actions" aria-label="老婆端快捷操作">
          <button className="wife-action wife-action--primary" type="button" onClick={() => setSheet("task")}>
            <FilePenLine size={21} />
            发布任务
          </button>
        ...
        <a className="wife-scroll-cue" href="#wife-today" onClick={handlePageLink("today")}>
          <span aria-hidden="true">∨</span>
          <span>上滑查看今日事务</span>
        </a>
      </section>
      ...
      <section className="wife-today" id="wife-today" aria-label="今日事务">
        <img className="wife-today__portrait" src="/assets/wife/wife-main.png" alt="" />
        <div className="wife-today__shade" />
        <a className="wife-today-back" href="#wife-main" onClick={handlePageLink("main")}>
          <span aria-hidden="true">∧</span>
          <span>下滑返回主页</span>
        </a>
        ...
        <section className="wife-today-card wife-overview" aria-label="今日概览">
          <h2>今日概览</h2>
        ...
        <section className="wife-today-card wife-pending-panel" aria-label="待老妞处理">
          <h2>待老妞处理</h2>
        ...
        <section className="wife-today-card wife-quick-panel" aria-label="快速发布">
          <h2>快速发布</h2>
          <button className="wife-quick-task" type="button" onClick={() => setSheet("task")}>
            <ScrollText size={39} />
            <span>
              <strong>发布新任务</strong>
              <em>安排今日差事</em>
        ...
        <section className="wife-today-card wife-recent-panel" aria-label="最近动态">
          <h2>最近动态</h2>
      ...
      {subPage ? (
        <section className="wife-subpage" aria-label="老婆端子页面">
          <img className="wife-subpage__portrait" src="/assets/wife/wife-main.png" alt="" />
          <div className="wife-subpage__shade" />
          <button className="wife-subpage-back" type="button" onClick={closeSubPage}>
            <ChevronLeft size={20} />
            返回
          </button>
          ...
          {subPage === "tasks" ? (
            <div className="wife-subpage-inner">
              ...
              <button className="wife-subpage-hero-action" type="button" onClick={() => setSheet("task")}>
                <ScrollText size={34} />
                <span>
                  <strong>发布新任务</strong>
                  <em>安排老哥今日差事</em>
                ...
                {tasks.map((task) => (
                  <article key={task.id}>
                    ...
                    <div>
                      ...
                      <small>{task.deadline} · 经验 +{task.rewardExp}{task.rewardMoney ? ` · 零花钱 +${task.rewardMoney}` : ""}</small>
                    </div>
          ...
          {subPage === "review" ? (
            <div className="wife-subpage-inner">
              ...
              <div className="wife-subpage-stats">
                ...
                <article>
                  <strong>{tasks.filter((task) => task.status === "confirmed").length}</strong>
                  <span>已确认</span>
                </article>
              </div>
                ...
                {submittedTasks.length ? (
                  submittedTasks.map((task) => (
                    <article key={task.id}>
                      ...
                      <div>
                        <h2>{task.title}</h2>
                        <p>{task.submitNote || "老哥已提交完成结果，等待老妞大人裁定。"}</p>
                        <small>奖励：经验 +{task.rewardExp}{task.rewardMoney ? ` · 零花钱 +${task.rewardMoney}` : ""}</small>
                      </div>
                      <div className="wife-subpage-actions">
                        <button type="button" onClick={() => onRejectTask(task.id)}>打回</button>
                        <button type="button" onClick={() => onApproveTask(task.id)}>确认</button>
                      </div>
                    </article>
                  ))
                ) : (
            ...
            <div className="wife-subpage-inner">
              ...
              <div className="wife-subpage-list wife-subpage-list--benefits">
                {benefits.map((benefit) => {
                  ...
                  return (
                    <article key={benefit.id} className={locked ? "is-locked" : undefined}>
                      <div className="wife-subpage-icon">
                        <ShieldCheck size={25} />
                      </div>
                      <div>
                        ...
                        <small>Lv.{String(benefit.levelRequired).padStart(2, "0")} 解锁 · {benefit.frequency}</small>
                      </div>
                      {locked ? (
                        <span>未解锁</span>
                      ) : (
                        <button type="button" onClick={() => onApproveBenefit(benefit)}>恩准</button>
                      )}
                    </article>
                  );
                })}
            ...
            <div className="wife-subpage-inner">
              ...
              <div className="wife-record-timeline">
                <article>
                  <span />
                  <div>
                    <h2>当前职务：Lv.{String(role.level).padStart(2, "0")} {role.title}</h2>
                    <p>经验 {progress.exp} / {requiredExp}，月薪 {role.salary}。</p>
                  </div>
                </article>
                {tasks.slice(0, 8).map((task) => (
                  <article key={task.id}>
                    <span />
                    <div>
                      <h2>{taskStatusLabel[task.status]} · {task.title}</h2>
                      <p>{task.resultText || task.submitNote || task.description}</p>
                    </div>
                  </article>
                ))}
              </div>
          ...
          {subPage === "order" ? (
            <div className="wife-subpage-inner">
              ...
              <div className="wife-order-panel">
                <div>
                  <strong>今日点餐权</strong>
                  <span>{progress.level >= 2 ? "可提交申请" : "尚需继续服役"}</span>
                </div>
              ...
              <div className="wife-subpage-list wife-subpage-list--order">
                {orderOptions.map((option) => (
                  <article key={option.name}>
                    ...
                    <button type="button" onClick={() => setSheet("benefit")}>申请</button>
                  </article>
                ))}
              </div>
            </div>
      ...
      {(activePage === "today" || subPage) ? (
        <nav className="wife-today-tabs" aria-label="今日事务导航">
          <a className={!subPage ? "active" : undefined} href="#wife-today" onClick={handleTodayTab}>
            <Crown size={28} />
            今日
          </a>
          <button className={subPage === "tasks" ? "active" : undefined} type="button" onClick={() => openSubPage("tasks")}>
            <ClipboardList size={28} />
            任务
          </button>
          <button className={subPage === "benefits" ? "active" : undefined} type="button" onClick={() => openSubPage("benefits")}>
            <ShieldCheck size={28} />
            权益
          </button>
          <button className={subPage === "records" ? "active" : undefined} type="button" onClick={() => openSubPage("records")}>
            <BookOpen size={28} />
            记录
          </button>
          <button className={subPage === "order" ? "active" : undefined} type="button" onClick={() => openSubPage("order")}>
            <Utensils size={28} />
            点餐
          </button>
        </nav>
      ...
      {sheet ? (
        <div className="modal-backdrop wife-sheet-backdrop" role="presentation">
          <section className="wife-sheet" role="dialog" aria-modal="true">
            <button className="icon-close" type="button" onClick={() => setSheet(null)} aria-label="关闭">
              <X size={22} />
            </button>
                ...
                <div className="wife-form-grid">
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => updateDraft("rewardExp", Number(event.target.value))}
                    />
                  </label>
                  <label>
                    零花钱
                    <input
                      ...
                      onChange={(event) => updateDraft("rewardMoney", Number(event.target.value))}
                    />
                  </label>
                </div>
