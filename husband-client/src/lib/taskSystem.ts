...
import { hydrateProgress, initialProgress, type GameProgress } from "../game/progression";
...
function normalizeTask(raw: unknown): Task | null {
  ...
  const source = value.source === "daily" ? "daily" : ("wife" satisfies TaskSource);
  const type = typeMap[String(value.type || value.urgency || "custom")] ?? "custom";
  const rewardExp = Number(value.rewardExp ?? value.exp ?? 0);
  const rewardMoney = Number(value.rewardMoney ?? value.money ?? 0);

  return {
    id: String(value.id || `task-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`),
    title,
    description: String(value.description || "由老妞大人发布，验收标准以老妞大人裁定为准。"),
    type: value.urgency === "urgent" ? "urgent" : type,
    source,
    rewardExp: Number.isFinite(rewardExp) ? Math.max(0, Math.trunc(rewardExp)) : 0,
    rewardMoney: Number.isFinite(rewardMoney) ? Math.max(0, Math.trunc(rewardMoney)) : 0,
    rewardBenefit: typeof value.rewardBenefit === "string" ? value.rewardBenefit : undefined,
    deadline: String(value.deadline || "今日完成"),
    status: statusMap[String(value.status || "todo")] ?? "todo",
    submitNote: typeof value.submitNote === "string" ? value.submitNote : undefined,
...
function hydrateTasks(raw: unknown): Task[] {
  ...
  const tasks = raw.map(normalizeTask).filter((task): task is Task => Boolean(task));
...
function stateFromUnknown(raw: unknown): TaskSystemState {
  if (!raw || typeof raw !== "object") {
    lastStateExtras = {};
    return {
      progress: hydrateProgress(readJson(PROGRESS_STORAGE_KEY) ?? initialProgress),
      tasks: hydrateTasks(readJson(TASKS_STORAGE_KEY)),
    };
  }

  const value = raw as Record<string, unknown>;
  const { exp, level, progress, rewardedTaskIds, tasks, totalExp, wallet, ...extras } = value;
